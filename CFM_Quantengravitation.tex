...
The $f(R)$ structure of the CFM Lagrangian ($\alpha_B = -\alpha_M/2$, $\alpha_T = 0$) enables direct numerical computation of perturbation observables using the hi\_class Boltzmann code \cite{Zumalacarregui2017}. We use the \texttt{propto\_omega} parametrization ($\alpha_i(a) = c_i \cdot \Omega_{\mathrm{DE}}(a)$), which ensures $\alpha_i \to 0$ at recombination, preserving BBN and the CMB damping tail. All cosmological parameters are fixed to the Planck 2018 best-fit values; the only additional parameter is $c_M$.
...
The recommended best-fit point $c_M = 0.0002$ yields $\Delta\chi^2_{\mathrm{tot}} = -0.2$ with $\sigma_8 = 0.826$ (only $+1.8\%$ above $\Lambda$CDM). Higher $c_M$ values improve $\chi^2$ further but at the cost of increased $\sigma_8$.
...
The combined parameter $S_8 = \sigma_8\sqrt{\Omega_m/0.3}$ is the primary observable from weak gravitational lensing surveys. The current observational landscape (as of 2026) shows a bifurcation:

\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Survey} & $S_8$ & \textbf{Tension with CFM} ($S_8 = 0.847$) \\
\midrule
Planck + ACT + SPT (CMB) & $0.836 \pm 0.013$ & $0.8\sigma$ \\
KiDS-Legacy (2025) & $0.815 \pm 0.021$ & $1.5\sigma$ \\
DES Y6 (2026) & $0.789 \pm 0.012$ & $4.8\sigma$ \\
HSC Y3 + DESI (2025) & $0.805 \pm 0.018$ & $2.3\sigma$ \\
eROSITA clusters & $0.860 \pm 0.010$ & $1.3\sigma$ \\
\bottomrule
\end{tabular}
\end{center}

The CFM prediction $S_8 = 0.847$ (at $c_M = 0.0002$) is \textit{consistent} with the CMB baseline, KiDS-Legacy, and eROSITA cluster counts, but in tension with DES~Y6. Crucially, the disagreement between KiDS-Legacy ($S_8 = 0.815$) and DES~Y6 ($S_8 = 0.789$) at the $\sim 1.5\sigma$ level suggests unresolved systematics in current weak lensing analyses. Euclid, with space-based image quality over 15{,}000\,deg$^2$, will be the decisive arbiter.

\subsection{DESI DR2 BAO Comparison}

The DESI Data Release~2 (2026) reports $w_0 = -0.42 \pm 0.21$ and $w_a = -1.75 \pm 0.58$, constituting a $3.1\sigma$ detection of dynamical dark energy. The CFM effective equation of state at $z = 0$ is $w_{\mathrm{eff}} \approx -0.33$, which lies within $0.4\sigma$ of the DESI $w_0$ measurement. Both the CFM and DESI data independently disfavor $w = -1$ (cosmological constant), providing qualitative support for the curvature feedback interpretation.
...
\item \textbf{Structure growth ($f\sigma_8$, $S_8$):} The growth rate at $z = 0.38$ \textit{improves} the BOSS LOWZ fit over $\Lambda$CDM. The $S_8 = 0.847$ prediction is consistent with Planck, KiDS-Legacy, and eROSITA (Section~\ref{subsec:s8_comparison}).
...
\item \textbf{Native CFM Boltzmann code:} The current implementation uses the \texttt{propto\_omega} parametrization as a proxy. A native implementation of the CFM expansion model $H(a)$ with the coupled scalaron-scalar field system would enable (a)~MCMC parameter estimation over $(\gamma, V_0, \phi_0)$ and (b)~predictions for the nonlinear matter power spectrum $P(k)$.
...
\item \textbf{DES Y6 tension:} The $4.8\sigma$ tension between the CFM $S_8$ prediction and DES~Y6 ($S_8 = 0.789$) must be monitored as Euclid results become available. If DES~Y6 systematics are confirmed, this tension would dissolve; if confirmed by Euclid, the CFM would need $\Omega_m < 0.31$ or a scale-dependent screening mechanism.
\end{enumerate}
...
Trotz des theoretischen Fortschritts verbleiben kritische Schritte:

\begin{enumerate}
\item \textbf{CMB-Leistungsspektrum:} Berechnung von $C_\ell$ aus den Störungsgleichungen der vollständigen Wirkung. Die "`Effective CDM"'-Analyse mit CLASS/hi\_class ergibt $\mathcal{P}_3/\mathcal{P}_1 = 0{,}4295$ (\textbf{exakter Planck-Match}) bei optimiertem $\beta_{\mathrm{early}} = 2{,}829$ (0{,}32\% Anpassung). Die verbleibende Herausforderung ist der $\theta_s$-Offset ($1{,}025$ vs.\ $1{,}041$), verursacht durch $w_{\mathrm{eff}} = -0{,}06$ des geometrischen Terms. Vollständige modifizierte Boltzmann-Gleichungen sind nötig.

\item \textbf{Geisteranalyse:} Verifikation, dass die Wirkung frei von Geisterinstabilitäten (Moden mit negativer kinetischer Energie) ist. Der $R^2$-Term führt das Skalaron ein, dessen Stabilität überprüft werden muss.

\item \textbf{Tests im Sonnensystem:} Die $R^2$-Modifikation erzeugt eine Yukawa-Korrektur zum Newtonschen Gesetz. Die Kopplung $\gamma$ muss klein genug sein, um den Beschränkungen aus dem Sonnensystem zu genügen.

\item \textbf{Numerische Verifikation:} Die hi\_class-Analyse bestätigt: BBN konsistent ($\Delta N_{\mathrm{eff}} \approx 0$), $P(k)$-Form korrekt, $\Omega_{m,\mathrm{eff}}(z=500) = 0{,}315$ = $\Lambda$CDM, und $\mathcal{P}_3/\mathcal{P}_1 = 0{,}4295$ bei optimiertem $\beta_{\mathrm{early}}$. Modified-Gravity-Scans (Horndeski $\alpha$-Funktionen) zeigen keinen Effekt bei $z = 1090$. Ein eigener Boltzmann-Integrator für die Störungsgleichungen des geometrischen Terms ist der nächste erforderliche Schritt.

\item \textbf{Quantengravitation:} Herleitung von $k$, $\Phi_0$, $\alpha$ und $\beta$ aus einem der mikroskopischen Rahmenwerke (LQG, Finsler, kausale Mengen) -- oder aus einem neuen, durch die $\tanh$-Struktur nahegelegten Rahmenwerk.
\end{enumerate}
