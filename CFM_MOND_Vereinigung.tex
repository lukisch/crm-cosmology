...
\end{equation}
This suggests that the dark matter density in $\Lambda$CDM ($\Omega_{\mathrm{CDM}} \approx 0.265$) is a geometric artifact: $3\sqrt{\pi}$ times the physical baryon density.
...
\end{itemize}
BBN is fully consistent ($\Delta N_{\mathrm{eff}} \approx 0$). The remaining challenge is the angular acoustic scale: $100\,\theta_s = 1.034$ vs.\ Planck's $1.041$ (offset reduced from 1.55\% to 0.69\%). A native CFM gravity model in hi\_class with time-dependent $\alpha_M(a)$ is needed for a definitive assessment.
...
\textit{Remaining challenge:} The $\theta_s$ offset (0.63\%) is the primary outstanding issue. A native CFM gravity model in hi\_class with time-dependent $\alpha_M(a)$ -- rather than the ad hoc constant parametrization -- is required for a definitive assessment.
