...
\textit{Resolution:} In the extended CFM, the geometric DM term $\alpha \cdot a^{-2}$ is \textit{not a material component} -- it is a property of spacetime geometry itself. At the epoch of recombination ($a \approx 10^{-3}$), the power-law term dominates strongly over the saturation term and provides gravitational potential wells that are:
\begin{itemize}
\item Decoupled from the photon-baryon fluid (they are geometric, not material)
\item Present at the correct amplitude to drive acoustic oscillations
...
\item \textbf{SN-only validation:} The Pantheon+ data probe the expansion history at $z \lesssim 2.3$. The model's predictions at high redshift (CMB at $z \approx 1100$) are extrapolations. The geometric DM term $\alpha \cdot a^{-2}$ grows as $a \to 0$, and its behavior at very early times must be checked for physical consistency.
...
\item \textbf{Big Bang Nucleosynthesis:} The geometric DM term modifies the expansion rate during BBN ($a \sim 10^{-9}$). The predicted primordial element abundances must remain consistent with observations.
