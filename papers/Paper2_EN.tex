...
\item \textbf{Paper~II} (this work): Extends the CFM to replace the particle dark sector with spacetime geometry, achieving a universe with exclusively baryonic matter content consistent with MOND. Introduces the Decaying Dark Geometry hypothesis, the running curvature coupling $\beta_{\mathrm{eff}}(a)$, the scale-dependent MOND coupling $\mu(a)$, and demonstrates joint SN + CMB + BAO compatibility ($\Delta\chi^2 = -5.5$ vs.\ $\Lambda$CDM, 6 parameters, zero EDE).
