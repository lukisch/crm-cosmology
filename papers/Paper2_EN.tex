...
\item \textbf{Extended CRM succeeds spectacularly:} Adding the geometric DM term $\alpha \cdot a^{-\beta}$ restores and \textit{exceeds} the fit quality, achieving $\Delta\chi^2 = -26.3$ versus $\Lambda$CDM -- better than both the standard $\Lambda$CDM \textit{and} the standard CRM by a wide margin.
...
\item \textbf{Overall:} $\Delta\chi^2 = -5.5$ (preferred $\mu(a)$ variant, no EDE) to $-6.1$ (constant $\mu = \sqrt{\pi}$ with EDE) -- the CRM+MOND \textit{outperforms} $\Lambda$CDM across the board.
\end{enumerate}
...
\end{equation}
Since the CRM Lagrangian~\eqref{eq:full_action} (Paper~III) has $\alpha_T = 0$ identically -- as confirmed by GW170817 \cite{Abbott2017gw} -- we obtain:
\begin{equation}
