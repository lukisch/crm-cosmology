...
\begin{abstract}
Papers~I--III of the Curvature Feedback Model (CFM) series demonstrated that a single scalar degree of freedom -- a curvature-coupled scalaron with P\"oschl-Teller saturation dynamics -- reproduces the cosmological dark sector ($\Delta\chi^2 = -5.5$ vs.\ $\Lambda$CDM on Pantheon+ in the preferred zero-EDE variant, CMB peak positions within $0.1\%$, $\alpha_T = 0$ exactly). However, the scalar sector alone cannot produce flat galactic rotation curves: its Yukawa fifth force enhances gravity by at most a factor $4/3$ at sub-Compton scales, insufficient to replace dark matter halos. The present paper closes this gap by introducing a second degree of freedom -- a timelike unit vector field $A_\mu$ -- whose necessity, structure, and coupling are derived from thermodynamic principles (maximum entropy production, hierarchical dissipation). We construct the teleodynamic coupling function $\mathcal{F}(A_\mu, \phi, T) = |T|/\rho_{\mathrm{crit}} \cdot \mathcal{B}(\phi) \cdot \mathcal{V}(A_\mu \partial^\mu \phi)$ with three components: a matter gatekeeper ($|T|/\rho_{\mathrm{crit}}$) ensuring BBN protection, a saturation control ($\mathcal{B} = \mathrm{sech}^2(\phi/\phi_0)$) linking MOND strength to expansion dynamics, and a vector projection ($A_\mu \partial^\mu \phi$) coupling local structure to the cosmological expansion rate. The model predicts $a_0 = cH_0/(2\pi)$ as an equilibrium condition -- not a fitted parameter -- and reproduces the Radial Acceleration Relation (RAR) and flat rotation curves without dark matter particles. We propose a zero-free-parameter test against the SPARC database (175 galaxies) and demonstrate that the $4/3$ convergence factor from $f(R)$ perturbation theory (Paper~III) coincides with the MOND phase-space ratio $V_3/V_2$ (Paper~II), constituting independent evidence for the nested hierarchy. The complete CFM action $S = S_{\mathrm{T1}} + S_{\mathrm{T2}}$ contains 8 terms, of which 5 are uniquely determined by the axioms, 1 is form-determined, 1 is strongly constrained by 6 conditions, and 4 parameters follow from data. No $\Lambda$, no dark matter particles -- only geometry.
\end{abstract}

\keywords{MOND, modified gravity, vector-tensor theory, radial acceleration relation, curvature feedback, dark matter alternative, galactic dynamics}
...
\thanks{Paper~IV in the CFM series. Companion papers: \cite{Geiger2026,Geiger2026b,Geiger2026c}.}
...
The Curvature Feedback Model (CFM) offers a framework that may resolve this coincidence. Papers~I--III \cite{Geiger2026,Geiger2026b,Geiger2026c} established:
\begin{itemize}
...
The vector field introduced here shares structural features with Bekenstein's TeVeS \cite{Bekenstein2004} and Skordis \& Z{\l}o\'snik's AeST \cite{Skordis2021}. However, the CFM vector sector differs in three key respects:
\begin{enumerate}
...
\begin{equation}
\underbrace{\text{Null space}}_{\text{Mother}} \to \underbrace{\phi \text{ (scalaron)}}_{\text{Daughter 1}} \to \underbrace{A_\mu \text{ (vector)}}_{\text{Daughter 2}} \to \underbrace{\text{Standard physics}}_{\text{Termination}}
\label{eq:hierarchy}
\end{equation}
...
\begin{table}[b]
\caption{\label{tab:terms}Status of terms in the CFM action. ``Determined'' means uniquely fixed by the axioms; ``form'' means the functional form is fixed but a parameter is free; ``constrained'' means subject to conditions listed in Sec.~\ref{sec:constraints}.}
\begin{ruledtabular}
\begin{tabular}{lll}
...
\end{equation}
compared to the empirical value $a_0^{\mathrm{obs}} = (1.20 \pm 0.02) \times 10^{-10}$\,m/s$^2$ \cite{McGaugh2016}. The $\sim 15\%$ discrepancy lies within the uncertainty introduced by the $\mathcal{O}(1)$ saturation factor $\mathcal{B}_0$ and is \textit{not} a fitted parameter.

\textit{Remark on the $2\pi$ factor:} The derivation above identifies $2\pi$ as arising from the time-to-space Fourier transform of the scalar dynamics. A rigorous derivation from the field equations would require solving the coupled scalar-vector system on a galactic background, which we defer to future work. For the MCMC analysis (Sec.~\ref{sec:mcmc}), we treat $a_0 = cH_0/(2\pi)$ as the model prediction and test whether the data are consistent with this value.
...
We demonstrate consistency of the CFM with this relation in two asymptotic limits:
...
A striking feature of the CFM is the convergence of the factor $4/3$ from two independent derivations:
...
That the same numerical factor emerges from perturbation theory (microscopic, Lagrangian-derived) and from phase-space geometry (macroscopic, thermodynamic) is non-trivial. It suggests that the $4/3$ factor reflects a deep structural property of the curvature feedback mechanism, not a coincidence.
...
\label{eq:mond_form}
\end{equation}
where the interpolation function $\mu(x)$ satisfies $\mu(x) \to 1$ for $x \gg 1$ (Newtonian) and $\mu(x) \to x$ for $x \ll 1$ (deep-MOND). In the CFM, $\mu$ is not postulated but emerges from the coupled system~\eqref{eq:combined_vec}--\eqref{eq:mod_poisson} with the Chameleon profile of $\varphi(r)$.

Extracting the explicit form of $\mu(x)$ requires solving the coupled scalar-vector-metric system as a nonlinear boundary-value problem for a given $\rho(r)$. This is a numerical computation that we defer to future work. For the SPARC test (Sec.~\ref{sec:mcmc}), we adopt the empirical interpolation~\eqref{eq:rar} with $a_0 = cH_0/(2\pi)$ fixed, which suffices to test the cosmological anchor independent of the exact interpolation form.
...
\item Compute $g_{\mathrm{CFM}}(g_{\mathrm{bar}}, a_0)$ using the RAR formula~\eqref{eq:rar}.
...
\item \textbf{Run~B:} CFM-deduction ($a_0 = cH_0/(2\pi)$, fixed). If Run~B achieves comparable or better $\chi^2$ than Run~A, the CFM prediction is confirmed: $a_0$ is \textit{not} a free galactic parameter but a cosmological equilibrium condition.
\end{itemize}
...
\begin{table}[b]
\caption{\label{tab:predictions}Predictions and consistency checks of the CFM vector sector.}
\begin{ruledtabular}
\begin{tabular}{lll}
...
The Aether Scalar Tensor (AeST) theory of Skordis \& Z{\l}o\'snik \cite{Skordis2021} is the closest existing framework to the CFM vector sector. Both theories employ a timelike unit vector field to generate MOND phenomenology within a relativistic framework. The key differences are:

\begin{enumerate}
\item \textbf{Origin:} AeST postulates the vector field; CFM derives it from thermodynamic hierarchy.
\item \textbf{$a_0$:} In AeST, $a_0$ is a free parameter; in CFM, $a_0 = cH_0/(2\pi)$ is deduced from the scalar-vector coupling.
\item \textbf{Dark energy:} AeST requires a cosmological constant; CFM replaces $\Lambda$ with the P\"oschl-Teller saturation of the scalaron.
\item \textbf{CMB compatibility:} AeST was demonstrated to fit the CMB power spectrum \cite{Skordis2021}; CFM has demonstrated CMB compatibility for the scalar sector (Paper~III) but the vector contribution to perturbations remains to be computed.
\end{enumerate}

A detailed comparison of the perturbation spectra (CMB $C_\ell$ and matter $P(k)$) between CFM and AeST would be highly informative but lies beyond the scope of this paper.
...
The deductive chain of the CFM is:
\begin{itemize}
...
\begin{enumerate}
\item \textbf{RAR interpolation function from field equations:} Section~\ref{sec:spherical} establishes the quasi-static framework, derives the modified Poisson equation~\eqref{eq:poisson_Xi}, and identifies the Chameleon-controlled nonlinear feedback loop~\eqref{eq:feedback_loop} as the mechanism generating MOND-like dynamics (Sec.~\ref{sec:nonlinear_origin}). However, the linearized reduction yields only a constant rescaling $g_A \propto g_N$, not the $\sqrt{g_N\,a_0}$ attractor; the $\sqrt{\phantom{x}}$-scaling must emerge from the full nonlinear system. The numerical BVP outlined in Sec.~\ref{sec:numerical_scheme} will determine whether the coupled system admits the expected MOND attractor and extract the emergent $\mu(x)$.

\item \textbf{Rigorous $2\pi$ derivation:} The factor $2\pi$ in $a_0 = cH_0/(2\pi)$ is identified as arising from the Fourier relationship between time-domain scalar dynamics and spatial-domain gravitational potential, but not yet derived from the coupled field equations. This calculation is closely related to item~1.
...
\item \textbf{Vector perturbation theory:} The contribution of $A_\mu$ to the CMB power spectrum and matter power spectrum has not been computed. Since $\rho_A = 0$ on FLRW (Sec.~\ref{sec:rho_A}), the vector sector does not affect the background cosmology, but its perturbations could modify the CMB at the percent level. Computing this is essential for full Planck-level consistency.
...
We have presented the complete Curvature Feedback Model action $S_{\mathrm{CFM}} = S_{\mathrm{T1}} + S_{\mathrm{T2}}$, incorporating both the scalar sector (dark energy, cosmological dark matter effects) and the vector sector (galactic MOND dynamics). The key results are:
...
\item The MOND scale $a_0 = cH_0/(2\pi)$ emerges as an order-of-magnitude prediction (within $\sim 15\%$ of the observed value), not a fitted parameter. Galaxies ``know'' the expansion rate because the vector field couples to the scalar field that drives the expansion. A rigorous derivation of the $2\pi$ factor from the field equations remains an open task.
...
\item The static spherical limit is derived explicitly (Sec.~\ref{sec:spherical}): the quasi-static decomposition $\phi = \bar{\phi}(t) + \varphi(r)$ reveals that the cosmological drift $\dot{\bar{\phi}} \sim H_0\phi_0$ is the physical bridge connecting expansion to galactic dynamics. The linearized reduction yields $g_A \propto g_N$ (constant rescaling); the deep-MOND scaling $g \sim \sqrt{g_N\,a_0}$ is expected to arise from the nonlinear Chameleon feedback (Sec.~\ref{sec:nonlinear_origin}), with the exact interpolation function $\mu(x)$ to be determined by numerical boundary-value-problem solutions.
...
\item A zero-dark-matter-parameter test against SPARC (175 galaxies) is proposed, with $a_0$ fixed by cosmology.
\end{enumerate}

The ontological picture is: two axioms (null-space fluctuation + thermodynamic optimization) $\to$ two degrees of freedom (scalar + vector) $\to$ three phenomena (dark energy, cosmological dark matter effects, MOND-compatible galactic dynamics) $\to$ one termination condition (standard physics below galactic scales). No $\Lambda$, no dark matter particles -- only geometry. The full validation of this picture requires numerical confirmation that the nonlinear Chameleon feedback produces the MOND interpolation function and a zero-free-parameter fit to the SPARC rotation curve database.
