...
\begin{enumerate}
\item \textbf{RAR interpolation function from field equations:} Section~\ref{sec:spherical} establishes the quasi-static framework, and the numerical BVP (Sec.~\ref{sec:bvp_results}) demonstrates that the full nonlinear system \textit{does} admit a MOND attractor at RAR slope~$= 0.500 \pm 0.001$. The remaining task is to extract the precise functional form of the emergent $\mu(x)$ and demonstrate percent-level consistency with the empirical McGaugh interpolation.

\item \textbf{Rigorous $2\pi$ derivation:} The factor $2\pi$ in $a_0 = cH_0/(2\pi)$ arises from the Fourier relationship between the temporal scalar dynamics ($\omega_H = H_0$) and the spatial gravitational potential ($k_0 = H_0/c$, with $a_0 = c \cdot k_0/(2\pi)$). A first-principles derivation from the coupled field equations is feasible with the existing BVP solver infrastructure but requires the full cosmological+galactic background computation.
...
\item The MOND scale $a_0 = cH_0/(2\pi)$ emerges as an order-of-magnitude prediction (within $\sim 13\%$, or $0.66\sigma$, of the observed value), not a fitted parameter. Galaxies ``know'' the expansion rate because the vector field couples to the scalar field that drives the expansion. A rigorous derivation of the $2\pi$ factor from the field equations remains an open task.
