...
\item \textbf{DESI DR2 tension and resolution via the Lagrangian theory:} The phenomenological Pantheon+ fit in this paper yields $w_{\mathrm{eff}} < -1$ (phantom regime). However, DESI DR2 \cite{DESI2025} reports $w_0 = -0.667^{+0.072}_{-0.066}$ (i.e., $w > -1$), placing it on the \textit{opposite side} of $w = -1$. We now regard this tension as resolved by the Lagrangian formulation in Paper~III~\cite{Geiger2026c}: the $f(R) = R + 2\gamma R^2$ sector generically yields $w \to -1$ from above, respecting the null energy condition. The phantom value $w = -1.355$ is an artefact of the phenomenological tanh fit with diagonal covariance, not a prediction of the fundamental theory. Independently, Kovacs~\cite{Kovacs2026} reaches the same conclusion via the Fisher information metric on a curvature-occupation field, proving $w \geq -1$ from metric positivity. We therefore regard $w \geq -1$ as the natural CRM prediction, consistent with both the Lagrangian theory and current observational constraints.
\end{enumerate}
