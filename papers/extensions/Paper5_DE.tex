...
Das zentrale Ergebnis ist das \textit{Saturation Theorem} (Theorem~\ref{thm:saturation}): Die Axiome A--D reduzieren die effektive kosmologische Antwort auf eine Abel-Koordinate, während die projektive Randquotienten-Normalisierung D' die $\tanh$-Normalform bis auf Umparametrisierung eindeutig auswählt. Das Papier ist wie folgt strukturiert:
...
Sei $T$ eine Theorie, deren effektive Antwortfunktion $S_T(g)$ die Axiome A--D erfüllt und die Antisymmetrie-Bedingung B'. Dann besitzt die normalisierte Antwort $\sigma(g)=S_T(g)/S_{\max}$ eine ungerade Abel-Koordinate $\phi:(-1,1)\to\mathbb{R}$ mit
\[
...
\end{equation}
Das Kompositionsgesetz ist dann als relativistische Geschwindigkeitsaddition $\mathcal{C}(x,y) = (x+y)/(1+xy)$ bestimmt, bis auf Reskalierung durch $S_{\max}$. Ohne D' bestimmen die Axiome A--D die Abel-/Randkragenstruktur, schließen aber nicht alle glatten nichtprojektiven Randkrägen aus.
\end{theorem}
...
\label{cor:exclusion}
Keines der folgenden Profile kann die Axiome~A--D plus die projektive Normalisierung D' gleichzeitig erfüllen, außer wenn es durch zulässige Reparametrisierung $u(g)$ auf die $\tanh$-Form reduzierbar ist:
\begin{enumerate}
...
\begin{enumerate}
\item \textbf{Ein konditionales No-Go-Theorem:} Jede kosmologische Theorie, die die Axiome~A--D plus die projektive Randquotienten-Normalisierung D' erfüllt, muss eine $\tanh$-artige Sättigung produzieren. Axiome~A--D allein liefern die Abel-/Randkragenstruktur.
...
\begin{enumerate}
\item Die \textit{Form} der Dynamik ($\tanh$) ist ein mathematisches Theorem, sobald der projektive Randquotient D' akzeptiert wird; ohne D' beweist das Theorem die Abel-/Randkragenstruktur.
...
Theorien, die die Axiome A--D erfüllen, sind auf ein Abel-/Randkragen-Sättigungsgesetz eingeschränkt. Theorien, die A--D und D' erfüllen, sind auf $\tanh$-Sättigung eingeschränkt, was eine konkrete, falsifizierbare Vorhersage für ihren kosmologischen Sektor liefert.
