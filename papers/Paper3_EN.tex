...
\date{February 2026 \\ \vspace{0.5em} \small \textit{Paper~III in the CFM series \cite{Geiger2026,Geiger2026b}}}
...
From the MCMC posterior (Section~\ref{sec:numerical}), $\alpha_{M,0} = 0.0013 \pm 0.0007$ at the present epoch. The scalaron mass in the cosmological background is determined by the second derivative of the $f(R)$ function:
\begin{equation}
...
\midrule
\multicolumn{7}{c}{\textit{MCMC best-fit (5 free parameters, 24 walkers, 9{,}600 samples)}} \\
...
A full MCMC exploration over five parameters $(\alpha_{M,0}, n, \omega_{\mathrm{cdm}}, \ln(10^{10}A_s), n_s)$ using \texttt{emcee} (24~walkers, 80~burn-in + 400~production steps, 9{,}600~samples total) yields a global best-fit of $\chi^2 = 6625.2$ ($\Delta\chi^2 = -3.6$ vs.\ $\Lambda$CDM) at $\alpha_{M,0} = 0.0024$, $n = 0.28$. The marginalized constraints are:
\begin{align}
\alpha_{M,0} &= 0.0013 \pm 0.0007 \qquad (1.78\sigma \text{ detection significance}) \notag \\
n &= 0.67 \pm 0.42 \notag \\
\omega_{\mathrm{cdm}} &= 0.11994 \pm 0.00029 \notag \\
\ln(10^{10}A_s) &= 3.044 \pm 0.002 \notag \\
n_s &= 0.9656 \pm 0.0024 \notag
\end{align}
The posterior satisfies $P(\alpha_{M,0} > 0) = 100\%$, indicating a consistent preference for modified gravity across all viable parameter combinations. The standard cosmological parameters ($\omega_{\mathrm{cdm}}$, $A_s$, $n_s$) are essentially uncorrelated with the modified gravity parameters ($|\rho| < 0.02$), confirming that the cfm\_fR extension is a clean, perturbative addition to $\Lambda$CDM. The strong anti-correlation between $\alpha_{M,0}$ and $n$ ($\rho = -0.63$) reflects the expected degeneracy: both parameters control the effective amplitude of modified gravity.

The full posterior distribution is shown in Figure~\ref{fig:corner}. The 2D contours (68\% and 95\% credible regions) reveal a characteristic ``banana-shaped'' degeneracy between $\alpha_{M,0}$ and $n$: a larger amplitude $\alpha_{M,0}$ is compensated by a smaller growth exponent $n$, maintaining a nearly constant effective modification $\alpha_M(a=1) = \alpha_{M,0} \cdot n / (1 + \alpha_{M,0})$. Crucially, the modified gravity parameters are uncorrelated with all standard cosmological parameters (all $|\rho| < 0.11$), demonstrating that the cfm\_fR extension does not introduce parameter degeneracies with the $\Lambda$CDM sector.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{cfm_contour.png}
\caption{Corner plot of the cfm\_fR MCMC posterior (9{,}600 samples from 24 walkers). Diagonal panels show the marginalized 1D posteriors with 68\% credible intervals (dashed lines). Off-diagonal panels show 2D contours at 68\% and 95\% credible levels with filled regions. Red crosses mark the best-fit point ($\chi^2 = 6625.2$, $\Delta\chi^2 = -3.6$ vs.\ $\Lambda$CDM). The anti-correlation $\rho(\alpha_{M,0}, n) = -0.63$ is clearly visible, while all cross-correlations between MG and standard parameters satisfy $|\rho| < 0.11$.}
\label{fig:corner}
\end{figure}
...
\item \textbf{Paper~II} \cite{Geiger2026b}: MOND unification, extended CFM, exclusively baryonic matter content with geometric dark sector replacement, geometric dark sector replacement. Running curvature coupling $\beta_{\mathrm{eff}}(a)$. Validated against Pantheon+ + Planck CMB + 9 BAO measurements jointly ($\Delta\chi^2 = -5.1$ vs.\ $\Lambda$CDM; $\ell_A = 301.477$, $\mathcal{R} = 1.7502$).
...
\begin{enumerate}
\item \textbf{Full CMB power spectrum (TT + TE + EE):} The hi\_class analysis with $\alpha_B = -\alpha_M/2$ (the $f(R)$ relation from the $R^2$ Lagrangian) achieves $\Delta\chi^2 = -2.7$ (grid scan) and $\Delta\chi^2 = -3.6$ (MCMC best-fit) against $\Lambda$CDM over 6{,}405 Planck data points (Section~\ref{sec:numerical}). The polarization spectra (TE, EE) are fully compatible. The CMB peak observables ($\ell_1 = 220$, $\mathcal{P}_3/\mathcal{P}_1 = 0.4295$) are reproduced exactly.
...
\item \textbf{Full MCMC estimation (completed):} The native \texttt{cfm\_fR} model yields $\alpha_{M,0} = 0.0013 \pm 0.0007$ ($1.78\sigma$ detection significance, $P(\alpha_{M,0} > 0) = 100\%$) from a 5-parameter MCMC with 9{,}600 samples against Planck TT+TE+EE (Section~\ref{sec:numerical}). The MCMC best-fit improves the $\chi^2$ by $-3.6$ relative to $\Lambda$CDM while leaving standard cosmological parameters at their Planck values. The result constitutes a \textit{hint} of modified gravity, not yet a formal detection ($> 2\sigma$).
