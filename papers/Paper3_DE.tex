...
\item \textbf{Volle MCMC-Parameterabsch\"atzung (abgeschlossen):} Das native \texttt{cfm\_fR}-Modell liefert $\alpha_{M,0} = 0{,}0013 \pm 0{,}0007$ ($1{,}78\sigma$ Detektionssignifikanz, $P(\alpha_{M,0} > 0) = 100\%$) aus einer 5-Parameter-MCMC mit 9.600~Samples gegen Planck TT+TE+EE. Der MCMC-Best-fit verbessert das $\chi^2$ um $-3{,}6$ gegen\"uber $\Lambda$CDM, wobei die kosmologischen Standardparameter auf ihren Planck-Werten verbleiben. Das Ergebnis stellt einen \textit{Hinweis} auf modifizierte Gravitation dar, noch keinen formalen Nachweis ($> 2\sigma$). Die marginalisierte Korrelationsmatrix zeigt, dass $\alpha_{M,0}$ und $n$ stark anti-korreliert sind ($\rho = -0{,}63$), w\"ahrend die MG-Parameter von den kosmologischen Standardparametern entkoppelt sind ($|\rho| < 0{,}02$).
